...
app.post("/get-resume", async (req, res) => {
  ...
  let userDoc = await DB.collection("users")
    .findOne({ email: email })
  const USERID = userDoc._id.toString();
  let resumeDoc = await DB.collection("resume")
    .findOne({ userid: USERID })
  if (resumeDoc) {
    delete resumeDoc._id;
    delete resumeDoc.userid;
    res.send({success: true, resumeDoc});
  } else {
...
app.post("/create-pdf", (req, res) => {
  ...
  const file = { content: pdfTemplate(req.body) };
  // fs.writeFileSync('./index.html', file.content);
  pdf.generatePdf(file, options).then(output => {
    console.log(output);
    fs.writeFileSync("Resume.pdf", output);
